 ...
 * When facingRight is false, the sprite is mirrored horizontally via canvas transform.
 ...
 */
export function drawBody(ctx, x, y, faceParams, actionState = 'idle', facingRight = true) {
  ...
  if (!facingRight) {
    ctx.save();
    // Mirror around the horizontal centre of the sprite
    ctx.translate(px + FIGHTER_W / 2, 0);
    ctx.scale(-1, 1);
    ctx.translate(-(px + FIGHTER_W / 2), 0);
  }
